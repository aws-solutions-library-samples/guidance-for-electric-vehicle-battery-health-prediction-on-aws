...
export async function handler(event?: any, context?: any) {
  ...
  let pipeId: string = "";
  try {
    const checkpoint = event.queryStringParameters["checkpoint"];
    pipeId = event.queryStringParameters["uuid"];

    // Kick off glue job with new checkpoint
    ...
    data.push(`Preprocessing initiated with checkpoint at ${checkpoint}`);

    await update_ddb(pipeId, {
      PreProcessingId: run["JobRunId"],
      PipelineRetraining: true,
      DataUploadedAt: new Date().toISOString(),
      PluginUploadedAt: new Date().toISOString(),
