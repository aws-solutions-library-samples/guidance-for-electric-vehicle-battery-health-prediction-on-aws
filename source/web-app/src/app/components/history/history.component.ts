...
export class HistoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    Auth.currentUserInfo().then((user) => {
      ...
      this.apiService
        .GetPipelinesByUser(username)
        .then((data: GetPipelinesByUserQuery[]) => {
          this.pipelineData = data
            ...
            .sort(
              (d1: GetPipelinesByUserQuery, d2: GetPipelinesByUserQuery) => {
                return (
                  new Date(String(d2.DataUploadedAt)).getTime() -
                  new Date(String(d1.DataUploadedAt)).getTime()
                );
              }
            );
          this.segmentedPipelineData = this.pipelineData.slice(
            0,
            this.itemsPerPage
          );
          ...
          this.totalPages = Math.ceil(
            this.pipelineData.length / this.itemsPerPage
          );
  ...
  goBack() {
    if (this.currentPage !== 1) {
      this.currentPage--;
      this.segmentedPipelineData = this.pipelineData.slice(
        this.lastShownIndex - this.itemsPerPage * 2,
        (this.lastShownIndex -= this.itemsPerPage)
      );
      this.emptyRows = [];
    }
  ...
  navigateToTracker(pipeline: any) {
    if (
      pipeline.PipelineStatus == "PIPELINE_FINISHED" ||
      pipeline.PipelineRetraining
    ) {
      this.router
        .navigate(["/dashboard"], { queryParams: { uuid: pipeline.Id } })
        .then();
    } else {
  ...
  private createEmptyRows() {
    if (this.segmentedPipelineData.length < this.itemsPerPage) {
      this.emptyRows = new Array(
        this.itemsPerPage - this.segmentedPipelineData.length
      );
    } else {
      this.emptyRows = [];
    }
