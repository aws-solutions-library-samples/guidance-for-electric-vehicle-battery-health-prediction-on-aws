...
export class TrackerComponent implements OnInit {
  ...
  availableSteps = [
    ...
    },
    {
      ...
      status: "IMPORTING_DATASET",
      timeInMs: 0,
    },
    {
      id: 4,
      title: "Training Predictor",
      status: "TRAINING_PREDICTOR",
      timeInMs: 0,
    },
    {
      id: 5,
      title: "Generating Forecast",
      status: "GENERATING_FORECAST",
      timeInMs: 0,
    },
    {
      id: 6,
      title: "Exporting Predictions",
      status: "EXPORTING_PREDICTIONS",
      timeInMs: 0,
    },
    {
      id: 7,
      title: "Cleaning Exports",
      status: "CLEANING_EXPORTS",
      timeInMs: 0,
    },
    {
      id: 8,
  ...
  ngOnInit(): void {
    if (!this.pipelineId) this.pipelineId = this.id;
    this.getPipelineInfo();
    this.apiService.PipelineSubListener.subscribe(
      (pipelineData: any) => {
        if (
          pipelineData &&
          pipelineData.value.data.pipelineSub?.Id === this.pipelineId
        ) {
          this.pipelineStatus =
            pipelineData.value.data.pipelineSub?.PipelineStatus;
          this.currentStep =
            this.availableSteps.find(
              (step: any) => step.status === this.pipelineStatus
            )?.id ?? this.currentStep;
          if (this.pipelineStatus === "PIPELINE_FINISHED") {
            ...
            this.router
              .navigate(["/dashboard"], {
                queryParams: { uuid: this.pipelineId },
              })
              .then();
          } else {
            this.getPipelineInfo();
      ...
      switch (stepId) {
        case 1: {
          return this.calculateTimeDiff(
            this.pipelineData.DataUploadedAt,
            this.pipelineData.PluginUploadedAt,
            1
          );
        }
        case 2: {
          return this.calculateTimeDiff(
            this.pipelineData.ProcFinishedAt,
            this.pipelineData.PluginUploadedAt,
            2
          );
        }
        case 3: {
          return this.calculateTimeDiff(
            this.pipelineData.DataImportedAt,
            this.pipelineData.ProcFinishedAt,
            3
          );
        }
        case 4: {
          return this.calculateTimeDiff(
            this.pipelineData.TrainingFinishedAt,
            this.pipelineData.DataImportedAt,
            4
          );
        }
        case 5: {
          return this.calculateTimeDiff(
            this.pipelineData.ForecastGeneratedAt,
            this.pipelineData.TrainingFinishedAt,
            5
          );
        }
        case 6: {
          return this.calculateTimeDiff(
            this.pipelineData.ForecastExportedAt,
            this.pipelineData.ForecastGeneratedAt,
            6
          );
        }
        case 7: {
          return this.calculateTimeDiff(
            this.pipelineData.CleaningFinishedAt,
            this.pipelineData.ForecastExportedAt,
            7
          );
        }
