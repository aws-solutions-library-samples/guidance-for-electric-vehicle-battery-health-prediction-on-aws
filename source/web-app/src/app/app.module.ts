  ...
  return () => {
    return metadataService.getConfig().pipe(
      map((config: any) => {
        ...
        (environment["NG_APP_DATASET_URI"] =
          config.bucketUrl + "/cdk-assets/raw_dataset.csv"),
          (environment["NG_APP_PLUGIN_URI"] =
            config.bucketUrl + "/cdk-assets/processing_plugin.py");
